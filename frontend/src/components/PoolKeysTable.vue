...
import { ref } from 'vue'
import type { PoolKey, KeyStatus } from '@/types'
...
function getSuccessRateValue(key: PoolKey): number | null {
  const st = getStatus(key)
  if (!st || st.success_rate == null) return null
  return Math.round(st.success_rate * 100)
}

function getSuccessRate(key: PoolKey): string {
  const rate = getSuccessRateValue(key)
  return rate == null ? '-' : String(rate)
}

function getSuccessRateLabel(key: PoolKey): string {
  const rate = getSuccessRateValue(key)
  return rate == null ? '-' : `${rate}%`
}

function getProgressValue(key: PoolKey): number {
  const rate = getSuccessRateValue(key)
...
function getRateClass(rate: string): string {
  if (rate === '-') return 'rate-none'
  const n = parseInt(rate)
  if (n >= 80) return 'rate-good'
  if (n >= 50) return 'rate-warn'
  return 'rate-bad'
}
      ...
      >
        ...
        <section class="health-panel">
          <div class="health-rate">
            <div class="health-rate-top">
              <span class="metric-label">成功率</span>
              <span :class="getRateClass(getSuccessRate(key))" class="health-rate-value">
                {{ getSuccessRateLabel(key) }}
              </span>
            </div>
            <el-progress
              :percentage="getProgressValue(key)"
              :color="getRateColor(getSuccessRate(key))"
              :stroke-width="8"
              :show-text="false"
            />
          </div>
        ...
        <section class="pool-card-details">
          ...
          <div class="detail-row">
            <span class="detail-label">OpenAI URL</span>
            <span class="detail-value detail-url">{{ key.openai_url || '-' }}</span>
          </div>
          <div class="detail-row">
            <span class="detail-label">Claude URL</span>
            <span class="detail-value detail-url">{{ key.claude_url || '-' }}</span>
          </div>
    ...
    <el-table v-else :data="poolKeys" stripe class="full-table" empty-text="暂无号池 Key">
      ...
      <el-table-column label="成功率" width="140">
        <template #default="{ row }">
          <div class="rate-cell">
            <el-progress
              :percentage="getProgressValue(row)"
              :color="getRateColor(getSuccessRate(row))"
              :stroke-width="6"
              :show-text="false"
              class="rate-progress"
            />
            <span :class="getRateClass(getSuccessRate(row))" class="rate-text">
              {{ getSuccessRateLabel(row) }}
            </span>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="熔断器" width="90">
        <template #default="{ row }">
...
.rate-text { font-size: var(--text-sm); }
.rate-none { color: var(--info); }
